...
def generate_launch_description():
    ...
    urdf_name = 'drone.xacro'
    urdf = os.path.join(get_package_share_directory('drone'), 'urdf', urdf_name)
    control_yaml_file = os.path.join(get_package_share_directory(
        'drone'
    ), 'config', 'ros2_control.yaml')
    robot_desc = ParameterValue(
        Command(['xacro ', urdf, ' ', 'ros2_control_yaml:=', control_yaml_file]),
        value_type=str
    )
